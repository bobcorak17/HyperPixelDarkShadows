...
#
def subsolar_point(dt_utc):
    """ Compute the location of the sun where directly overhead (latitude, longitude in degrees) at the given UTC datetime
        using PyEphem. Returns (lat_deg, lon_deg).
    """
    # Use Ephem to get Sun position
    obs = ephem.Observer()
    obs.date = dt_utc
    sun = ephem.Sun(obs)
    ...
    lat_deg = math.degrees(sun.lat)
    lon_deg = math.degrees(sun.lon)
    return lat_deg, lon_deg
...
def sublunar_point(dt_utc: datetime):
    ...
    # Latitude and longitude in degrees
    lat_deg = math.degrees(moon.lat)
    lon_deg = math.degrees(moon.lon)
    return lat_deg, lon_deg
